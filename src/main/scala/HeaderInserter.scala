...
abstract class HeaderInserter[D, U, E, O, B <: Data](mode1024: Boolean, beatBytes: Int)
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    when(
      in.fire && (xDimensionCounter === (chirpNum - 1.U)) && (yDimensionCounter === (sampleNum - 1.U)) && (antennaCounter === (antennaNum - 1.U))
    ) {
      antennaCounter := 0.U
    }.elsewhen(
      in.fire && (xDimensionCounter === (chirpNum - 1.U)) && (yDimensionCounter === (sampleNum - 1.U)) && (antennaCounter < (antennaNum - 1.U))
    ...
    when(!modeSelection) {
      outputDataLastVal := Cat(
        Cat(0.U(8.W), 165.U(8.W)),
        Cat(antennaCounter.asTypeOf(UInt(8.W)), segmentCounter.asTypeOf(UInt(8.W)))
      )
    }.otherwise {
      outputDataLastVal := Cat(
        Cat(0.U(8.W), 165.U(8.W)),
        Cat(segmentCounter2.asTypeOf(UInt(8.W)), segmentCounter3.asTypeOf(UInt(8.W)))
      )
    }
